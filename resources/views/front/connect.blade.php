    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 bg-white shadow-sm sm:rounded-lg py-10">
            <div class="overflow-hidden flex flex-col gap-y-5">
                <h2 class="font-bold text-3xl">Connect Topup</h2>
            </div>
            ...
            <div class="flex flex-row justify-between items-center">
                ...
                <div class="flex flex-col gap-y-3 mt-4">
                    @can('topup connect')
                        @php
                            $latestTopup = Auth::user()->connectTopups()->orderByDesc('id')->first();
                        @endphp

                        {{-- Pending --}}
                        @if($latestTopup && !$latestTopup->is_paid && $latestTopup->status === 'pending')
                        ...
                        {{-- Ditolak => pesan + form --}}
                        @elseif($latestTopup && $latestTopup->status === 'rejected')
                            <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg">
                                Top-up Connect Anda <b>DITOLAK</b>. Silakan ajukan kembali.
                            </div>
                        ...
                        {{-- Belum ada topup & connect = 0 => Form --}}
